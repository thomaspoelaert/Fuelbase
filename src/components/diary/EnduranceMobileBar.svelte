<script>
  ...
  $: goalIntent = plan?.goal?.intent || plan?.config?.goalIntent || 'maintain';
...
<aside class="emb" class:expanded aria-label="Endurance nutrition plan">
  <button class="emb-summary" type="button" on:click={() => expanded = !expanded} aria-expanded={expanded}>
    ...
    {:else if plan}
      <div class="emb-main">
        <div class="emb-title-row"><strong>{remaining >= 0 ? `${remaining.toLocaleString()} kcal left` : `${Math.abs(remaining).toLocaleString()} kcal over`}</strong><span>{goalLabel(goalIntent)} · {consumed.toLocaleString()} / {target.toLocaleString()}</span></div>
  ...
  {#if expanded && plan}
    <div class="emb-detail">
      <div class="emb-energy">
        <div><span>Foundation</span><strong>{Math.round(plan.calories?.foundation ?? plan.calories?.base ?? 0).toLocaleString()}</strong></div>
      ...
      <div class="emb-goal-note">
        <span class="material-symbols-rounded">shield</span>
        <span><strong>{goalLabel(goalIntent)}</strong>{#if plan.calories?.goalAdjustment} · {plan.calories.goalAdjustment > 0 ? '+' : ''}{Math.round(plan.calories.goalAdjustment)} kcal outside training fuel{/if}</span>
      </div>
      ...
      {:else}
        <div class="emb-rest"><span class="material-symbols-rounded">self_improvement</span><span>Rest day — foundation nutrition target only.</span></div>
      {/if}
      ...
      {#if plan.forward48h?.length}
        <div class="emb-forward">
          <div class="emb-forward-head"><span>Next 48h</span><span>carb periodization</span></div>
          <div class="emb-forward-days">
            {#each plan.forward48h as day}
              <div class="emb-forward-day">
                <div><strong>{shortDay(day.date)}</strong><span>{day.workouts?.[0]?.name || 'Rest'}</span></div>
                <div><strong>{Math.round(day.targetKcal || 0).toLocaleString()}</strong><span>{Math.round(day.carbsG || 0)}g C</span></div>
              </div>
            {/each}
          </div>
        </div>
...
<style>
  ...
  .emb-summary { width:100%; min-height:54px; padding:8px 9px; display:flex; align-items:center; gap:9px; color:var(--text-1); text-align:left; }
  ...
  .emb-detail { padding:0 10px 10px;display:flex;flex-direction:column;gap:8px;border-top:1px solid var(--border); max-height:min(62vh,520px); overflow:auto; }
  ...
  .emb-goal-note { display:flex;align-items:center;gap:5px;color:var(--text-3);font-size:8.5px;padding:0 2px; }
  .emb-goal-note .material-symbols-rounded { color:var(--accent);font-size:13px; }
  .emb-goal-note strong { color:var(--text-2); }
  ...
  .emb-forward { display:flex;flex-direction:column;gap:5px;padding-top:2px; }
  .emb-forward-head { display:flex;justify-content:space-between;gap:8px;color:var(--text-3);font-size:8px; }
  .emb-forward-head span:first-child { color:var(--text-2);font-weight:700; }
  .emb-forward-days { display:grid;grid-template-columns:repeat(2,1fr);gap:5px; }
  .emb-forward-day { min-width:0;display:flex;justify-content:space-between;gap:6px;padding:7px;border-radius:10px;background:var(--surface-2); }
  .emb-forward-day>div { min-width:0;display:flex;flex-direction:column;gap:1px; }
  .emb-forward-day>div:last-child { align-items:flex-end; }
  .emb-forward-day strong { font-size:8.5px; }
  .emb-forward-day span { color:var(--text-3);font-size:7.5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:92px; }
